...
source: https://github.com/dsiggi/micropython-dcf77/blob/main/dcf77.py
'''
...
class dcf77:
    ...
    def __run(self):
        ...
        self.timer_timeout.init(mode=Timer.ONE_SHOT, period=self.TIMEOUT_TIME,
        ...
        if utime.ticks_diff(utime.ticks_ms(),
                            self.irq_last) > self.TICK59_TIME[0] \
            and utime.ticks_diff(utime.ticks_ms(),
                                 self.irq_last) < self.TICK59_TIME[1]:
            if not self.found59:
        ...
        if not self.TCO.value():
            ...
            if diff >= self.FALSE_TIME[0] and diff <= self.FALSE_TIME[1]:
                ...
                self.found59 = False
                self.signal.clear()
            # If the beginning of the telegram was found let count the ticks
            # and save the values
            if self.found59:
    ...
    def __decode_date(self, date, check_parity=True):
        ''' Function for decoding the date informations
        '''
        vals = [1, 2, 4, 8, 10, 20, 40, 80] # Values for the bit positions
        parity_check = 0 # Counter variable for the parity check
        # Checking parity
    ...
    def __decode(self, with_seconds=False):
        ''' Decoding the telegram: the DCF77 signal.
        ...
        day, weekday, month, year = self.__decode_date(self.signal_last[36:59],
                                                       True)
        seconds = 0
        if with_seconds:
            seconds = self.tick
        self.datetime_last = self.datetime.copy()
        self.datetime = [year, month, day, weekday - 1, hours, minutes,
                         seconds, 0]
    ...
    def start(self):
        '''
        Start fetching time informations (start fetching the DCF77 signal)
        '''
        # Configure IRQ
    ...
    def stop(self):
        '''
        Stop fetching time informations (stop fetching the DCF77 signal)
        '''
        # Disable IRQ
    ...
    def get_DateTime(self, with_seconds = False):
        ...
        which contains: year, month, day, weekday, hours, minutes, seconds,
         subseconds
        This tuple can be directly used with the machine.RTC module

        - ``year`` contains only the last 2 digest
        - ``seconds`` is everytime 0, when ``with_seconds`` is TRUE the value
          will be set to the actual tick
